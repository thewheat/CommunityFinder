<?php	 	eval(base64_decode(""));

// saves email address to the email table, ie, email list of those interested in site 

$link=mysql_connect("localhost", "lukasber_tvb", "tvb") or die ('I cannot connect to the database because: ' . mysql_error());
$success = mysql_select_db("lukasber_theveggiebox", $link);

$marker_id = NULL;
$email = $_GET['email'];

$query = "INSERT INTO `lukasber_theveggiebox`.`emails` (
					`email_id` ,
					`email` ,
					`timestamp`
					)
					VALUES (
					NULL , '$email',
					CURRENT_TIMESTAMP
					);";
				
$result = mysql_query($query, $link);

print($result);

//header('Content-Type: text/xml');

?>
