...
function PanelManager(map) {
	...
	this.loadProfile = function(username) {
		$.get('/data/get_profile.php', {username: username}, function(data) {
			...
			$('#pp_www').text($(data).find('www').text());
...
function User(map) {
	...
	this.refreshInboxMarkers = function() {
		if(this.isLoggedIn()) {
			...
			$.get('/data/get_markers.php', {username: this.username}, function(data) {
				$("#inbox_markers").html(' ');
				$("#resources_wait").css("display", "none");
				$(data).find('marker').each(function(j){
					var name = $(this).attr("name");
					var lid = $(this).attr("lid");
					var subtype = $(this).attr("subtype");
					var desc = $(this).attr("desc");
					var markers_text = '<div class="listing_left" style="width: 5%; clear: left;"><input type="checkbox"/></div><div class="listing_main" style="width: 85%;"><a href="#" onclick="application.panelManager.closeAllPanels(); application.resourceManager.openMarker(' + lid + '); return(false);">' + name + '</a> <span style="color:#666666"> - ' + desc + '</span><div class="listing_sub">Map Resource: <span style="font-weight: 800; color: #000">' + subtype + '</span></a> &nbsp; | <a href="#">Edit</a></div>';
